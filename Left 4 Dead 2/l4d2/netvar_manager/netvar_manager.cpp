...
static void add_props_for_table(netvar_table_map & table_map, const uint32_t table_name_hash,
	...
	std::map< uintptr_t, std::string > > & var_dump, const size_t child_offset = 0);
static void initialize_props(netvar_table_map& table_map);

uintptr_t netvar_manager::get_netvar(const uint32_t table,
	const uint32_t prop) {
...
static void initialize_props(netvar_table_map& table_map)
{
	...
	for (auto client_class = g_l4d2.m_client->get_client_class();
		client_class;
